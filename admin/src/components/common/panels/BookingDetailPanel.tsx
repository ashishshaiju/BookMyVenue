...
export function BookingDetailPanel({ data: rawData }: { data: Record<string, unknown> }) {
  ...
  const statusColor = STATUS_COLORS[data.status] || 'bg-gray-500/10 text-gray-500';
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-4 rounded-xl bg-background/50 border border-border">
        <div>
          <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
            Booking <span className="font-mono text-muted-foreground text-base">#{data._id?.slice(-6).toUpperCase()}</span>
          </h2>
        ...
        <Badge className={`${statusColor} border-none px-3 py-1 text-sm font-medium`}>
          {data.status}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="p-5 space-y-4 bg-background/50">
          ...
          <div className="grid grid-cols-2 gap-y-3">
            ...
            <div>
              <span className="text-muted-foreground block text-xs uppercase tracking-wider mb-1">Time Slots</span>
              <span className="font-medium text-foreground">
        ...
        <Card className="p-5 space-y-4 bg-background/50">
          ...
          <div className="grid grid-cols-2 gap-y-3">
            <div>
              ...
              <span className="text-lg font-bold text-green-500">₹{data.price}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <Badge variant="outline" className={data.paymentReference ? 'text-green-500 border-green-500/30' : ''}>
                {data.paymentReference ? 'Completed' : 'Pending'}
              </Badge>
            </div>
            {data.paymentReference && (
              <div>
                <span className="text-muted-foreground block text-xs uppercase tracking-wider mb-1">Transaction ID</span>
        ...
        <Card className="p-5 space-y-4 bg-background/50">
          ...
          <div className="space-y-3">
            <div>
              <span className="text-muted-foreground block text-xs uppercase tracking-wider mb-1">Venue Name</span>
              <span className="font-medium text-foreground">{data.venue?.name || 'N/A'}</span>
            </div>
            ...
            <div className="text-xs">
              <span className="text-muted-foreground block uppercase tracking-wider mb-1">Venue ID</span>
              <span className="font-mono bg-accent/50 px-2 py-1 rounded">{data.venue?._id || data.venueId}</span>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-5 space-y-4 bg-background/50">
          ...
          <div className="space-y-3">
            <div>
              ...
              <span className="font-medium text-foreground">{data.bookerInfo?.name || 'N/A'}</span>
            </div>
            {(data.bookerInfo?.email || data.bookerInfo?.phone) && (
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <span className="text-foreground">{data.bookerInfo?.email || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-xs uppercase tracking-wider mb-1">Phone</span>
                  <span className="text-foreground">{data.bookerInfo?.phone || 'N/A'}</span>
                </div>
              </div>
            )}
            <div className="text-xs">
              <span className="text-muted-foreground block uppercase tracking-wider mb-1">User ID</span>
              <span className="font-mono bg-accent/50 px-2 py-1 rounded">{data.userId}</span>
            </div>
          </div>
        </Card>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.cancellationReason && (
              <div>
                ...
                <span className="text-foreground">{data.cancellationReason}</span>
              </div>
            )}
          </div>
        </Card>
