...
export function VenueDetailPanel({
  ...
  const allImages = [data.coverImage, ...(data.galleryImages || [])].filter(Boolean) as string[];
  ...
  const hasStatusAlerts = (data.status === "Rejected" && data.rejectionReason) || (data.status === "Suspended" && data.suspensionReason);
      ...
      {data.coverImage && (
        ...
        >
          ...
          <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
            <div>
              <h2 className="text-2xl font-bold text-white shadow-sm">{data.name}</h2>
            ...
            <div className="bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/20 text-white flex flex-col items-end">
               <span className="text-xs text-white/80 uppercase tracking-wider font-semibold mb-0.5">Capacity</span>
               <span className="font-medium text-sm flex items-center gap-1.5">
                 <Users className="w-4 h-4" /> {data.maxCapacity || data.capacity || "N/A"}
               </span>
            </div>
          </div>
        </button>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              Gallery Photos <Badge variant="secondary" className="px-1.5 py-0 rounded-full">{data.galleryImages.length}</Badge>
            </span>
          </div>
        ...
        <div className="space-y-3">
          {data.status === "Rejected" && data.rejectionReason && (
            <div className="p-4 rounded-lg bg-destructive/10 border border-destructive/20 flex gap-3 items-start">
              ...
              <div>
                ...
                <p className="text-destructive/90 text-sm leading-relaxed">{data.rejectionReason}</p>
              </div>
            </div>
          )}
          {data.status === "Suspended" && data.suspensionReason && (
            <div className="p-4 rounded-lg bg-amber-500/10 border border-amber-500/20 flex gap-3 items-start">
              ...
              <div>
                <h4 className="font-semibold text-amber-700 mb-1">Suspension Reason</h4>
                <p className="text-amber-700/90 text-sm leading-relaxed">{data.suspensionReason}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-5 shadow-sm space-y-4">
          ...
          <dl className="space-y-2.5">
            <div className="flex justify-between items-center">
              ...
              <dd className="font-medium capitalize">{data.venueType || "—"}</dd>
            </div>
            <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-start pt-1 border-t border-border/50">
                ...
                <dd className="flex gap-1 flex-wrap justify-end pl-4">
                  {data.workingDays.map(day => (
                    <Badge key={day} variant="secondary" className="text-[10px] uppercase">{day.slice(0,3)}</Badge>
                  ))}
                </dd>
              </div>
            )}
        ...
        <Card className="p-5 shadow-sm space-y-4">
          ...
          <dl className="space-y-2.5">
            ...
            <div className="flex justify-between items-start pt-1 border-t border-border/50">
              <dt className="text-muted-foreground mt-0.5 w-20 shrink-0">Address</dt>
              <dd className="text-right leading-tight">{data.address || "—"}</dd>
            </div>
          </dl>
        </Card>
      </div>
        ...
        {data.bookingType === "fixedBooking" && fixedPackages && fixedPackages.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {fixedPackages.map((pkg, i) => (
              <div key={i} className="bg-secondary/30 rounded-lg p-3 border border-border flex flex-col gap-1.5">
                <div className="flex justify-between items-start mb-1">
                  <span className="font-semibold text-sm">{pkg.slotName || pkg.name || `Slot ${i+1}`}</span>
                  <Badge variant="default" className="bg-green-600/10 text-green-700 hover:bg-green-600/20 border-0">
                    ₹{pkg.price}
                  </Badge>
                </div>
            ...
            {data.pricing && (
               <div className="flex gap-4">
                 <div className="bg-secondary/30 px-4 py-2 rounded-lg border border-border">
                    <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Pricing Type</p>
                    <p className="font-medium capitalize">{data.pricing.pricingType || "—"}</p>
                 </div>
                 <div className="bg-secondary/30 px-4 py-2 rounded-lg border border-border">
                    <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Base Price</p>
                    <p className="font-semibold text-primary">₹{data.pricing.basePrice || "0"}</p>
                 </div>
               </div>
            )}
              ...
              <div className="overflow-x-auto border border-border rounded-lg">
                <table className="w-full text-left text-sm">
                  <thead className="bg-secondary/50 text-muted-foreground text-xs uppercase">
                    <tr>
                      <th className="px-4 py-2.5 font-medium border-b border-border">Time Range</th>
                      <th className="px-4 py-2.5 font-medium border-b border-border text-right">Price Modifier</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {pricingRules.map((rule, i) => (
                      <tr key={i} className="hover:bg-secondary/10">
                        ...
                        <td className="px-4 py-2 font-medium text-right text-amber-700">₹{rule.price}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <Card className="p-5 shadow-sm space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {data.amenities && data.amenities.length > 0 ? (
              ...
              <span className="text-muted-foreground italic text-sm">No amenities listed</span>
            )}
          </div>
        </Card>
      ...
      {data.description && (
        <Card className="p-5 shadow-sm space-y-3 bg-secondary/20">
          ...
          <div className="relative">
             <p className={`text-muted-foreground leading-relaxed whitespace-pre-wrap ${!isDescExpanded ? 'line-clamp-4' : ''}`}>
               {data.description}
             </p>
      ...
      <div className="border-t border-border mt-8 pt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground px-2">
         ...
         {data.avgRating !== undefined && (
            <div className="flex items-center gap-1.5 ml-auto">
              ...
              <span className="font-medium text-foreground">{data.avgRating.toFixed(1)}</span>
