      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-4 bg-background/50 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-y-1">
            ...
            <span>
              {data.capacity || "N/A"} (Max: {data.maxCapacity})
            </span>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-background/50 space-y-2">
          ...
          {data.pricing && (
            <div className="space-y-1">
              <div className="grid grid-cols-2 gap-y-1">
                <span className="text-muted-foreground">Type:</span>{" "}
                <span className="capitalize">{data.pricing.pricingType}</span>
                <span className="text-muted-foreground">Base Price:</span>{" "}
                <span>₹{data.pricing.basePrice}</span>
              </div>
              ...
              <ul className="list-disc list-inside space-y-1 text-xs">
                {fixedPackages.map(
                  (
                    ...
                    <li key={i}>
                      {pkg.name} ({pkg.startTime}-{pkg.endTime}): ₹{pkg.price}
                    </li>
                  ),
                )}
              </ul>
