...
export function Sidebar() {
  ...
  const navItems = NAV_CONFIG.filter((item) => profile && item.roles.includes(profile.role));
    ...
    <div className="flex h-screen w-64 flex-col bg-[var(--sidebar-bg)] border-r border-[var(--sidebar-border)] text-[var(--sidebar-text)]">
      ...
      <nav className="flex-1 space-y-1 px-4 py-4">
        {navItems.map((item) => {
          ...
          const toPath = typeof item.path === 'function' ? item.path(activeVenueId) : item.path;
          ...
          >
            ...
            <SelectContent>
              {venues.map((venue) => (
                ...
                >
                  {venue.name} {venue.status !== 'Approved' ? `(${venue.status})` : ''}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <div className="border-t border-[var(--sidebar-border)] p-4">
        <div className="flex items-center">
          ...
          <div className="ml-3 flex-1 overflow-hidden">
            ...
            <p className="truncate text-xs text-zinc-400 capitalize">{profile?.role}</p>
          </div>
          <Button variant="ghost" size="icon" onClick={handleSignOut} className="text-zinc-400 hover:text-white hover:bg-zinc-800" title="Sign Out">
