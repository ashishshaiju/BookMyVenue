    ...
    if (profile && user._id === profile._id) {
      ...
      openModal({
        ...
        component: () => (
          <div className="p-4 text-center">
            <p className="mb-4">
              This will generate a new random password and email it to{" "}
              <strong>{user.email}</strong>. Are you sure?
            </p>
            <div className="flex justify-center gap-4">
              <Button
                variant="outline"
              ...
              <Button
                variant="destructive"
                onClick={() =>
                  resetPasswordMutation.mutate({ userId: user._id }, {
                    onSuccess: () => {
                      success("Password reset successfully. Email dispatched.");
                      closeModal();
                    },
                    onError: (err) => {
                      const axiosErr = err as import("axios").AxiosError<{ message: string }>;
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            SuperAdmin control panel for all users.
          </p>
        </div>
      </div>
        ...
        <DialogContent className="rounded-2xl sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="owner_dashboard">
                    Owner Dashboard Ban
                  </SelectItem>
                  <SelectItem value="venue_creation">
                    Venue Creation Ban
                  </SelectItem>
                </SelectContent>
              </Select>
              <p className="text-xs text-[var(--text-secondary)]">
                {banDialog.scope === "full" &&
                  "Blocks all access to the platform"}
                {banDialog.scope === "commenting" &&
                  "Blocks review/comment creation"}
                {banDialog.scope === "owner_dashboard" &&
                  "Blocks owner dashboard access"}
                {banDialog.scope === "venue_creation" &&
            ...
            <div className="space-y-2">
              ...
              <Select
                value={banDialog.duration || "permanent"}
                onValueChange={(value) =>
                  setBanDialog({
                    ...banDialog,
                    duration: value as
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                banUserMutation.isPending || banDialog.reason.trim().length < 10
              }
            >
