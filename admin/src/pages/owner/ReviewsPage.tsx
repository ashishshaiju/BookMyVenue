...
const HIDE_REQUEST_VARIANT: Record<string, 'default' | 'secondary' | 'destructive' | 'outline'> = {
  none: 'outline',
  pending: 'secondary',
...
export default function ReviewsPage() {
  ...
  const [replyDialog, setReplyDialog] = useState<ReplyDialogState>({ open: false });
  ...
  const { data, isLoading } = useApiQuery<ReviewsResponse>(
    ...
    { method: 'GET', url: `${API_ENDPOINTS.OWNER_VENUE_BOOKINGS}/${venueId}/reviews?page=${page}&limit=10` },
    { staleTime: 0, enabled: !!venueId }
  );

  const replyMutation = useApiMutation<unknown, {
    venueId: string;
    reviewId: string;
    text: string;
  }>(
    ...
    }),
    {
      onSuccess: () => {
        ...
        void queryClient.invalidateQueries({ queryKey: QUERY_KEYS.OWNER_REVIEWS(venueId!) });
  ...
  const reportMutation = useApiMutation<unknown, {
    venueId: string;
    reviewId: string;
    reason: string;
  }>(
  ...
  const columns: ColumnDef<Review, unknown>[] = [
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{row.original.userId.email}</p>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-0.5">
          {[...Array(5)].map((_, i) => (
            <span key={i} className={i < row.original.rating ? 'text-yellow-500' : 'text-gray-300'}>
              ★
            </span>
          ))}
        </div>
    ...
    },
    {
      ...
      header: 'Comment',
      cell: ({ row }) => (
        <p className="text-sm max-w-xs truncate">{row.original.comment || '—'}</p>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        row.original.ownerReply ? (
          <div className="text-sm">
            <p className="text-gray-600 dark:text-gray-400 italic">{row.original.ownerReply.text}</p>
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-2">
          ...
          {row.original.hideRequestStatus !== 'none' && (
            <Badge variant={HIDE_REQUEST_VARIANT[row.original.hideRequestStatus]}>
              {row.original.hideRequestStatus === 'pending' ? 'Hide Requested' : `Hide ${row.original.hideRequestStatus}`}
            </Badge>
          )}
        </div>
      ),
  ...
  return (
    <div className="space-y-6 p-8 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground mt-1">All reviews left on your venue. Reply to reviews and report inappropriate content to admins.</p>
      </div>
      ...
      {/* Reply Dialog */}
      <Dialog open={replyDialog.open} onOpenChange={(open) => setReplyDialog({ open })}>
      ...
      <Dialog open={reportDialog.open} onOpenChange={(open) => setReportDialog({ open })}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Explain why this review should be hidden. An admin will review your request.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="report-reason">Reason (minimum 10 characters)</Label>
              <textarea
                id="report-reason"
                ...
                className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 resize-none"
              />
              <p className="text-xs text-muted-foreground">{reportReason.length}/500</p>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={reportMutation.isPending || reportReason.trim().length < 10}
            >
