...
export function useReplyToReview() {
  ...
  return useApiMutation<unknown, { venueId: string; reviewId: string; text: string }>(
    (vars) => ({
      method: 'POST',
    ...
    {
      onSuccess: (_, vars) => {
        queryClient.invalidateQueries({ queryKey: QUERY_KEYS.OWNER_REVIEWS(vars.venueId) });
      },
    }
  );
}
...
export function useReportReview() {
  const queryClient = useQueryClient();
  return useApiMutation<unknown, { venueId: string; reviewId: string; reason: string }>(
    (vars) => ({
      method: 'POST',
