...
export function useToggleUserStatus() {
  ...
  return useApiMutation<{ active: boolean }, { userId: string }>(
    (vars) => ({ url: `${API_ENDPOINTS.TOGGLE_USER_STATUS}/${vars.userId}/toggle-status`, method: 'PATCH' }),
    {
      onSuccess: () => {
...
export function useChangePassword() {
  return useApiMutation<unknown, unknown>(
    () => ({ url: API_ENDPOINTS.CHANGE_PASSWORD, method: 'PATCH' })
  );
}

export function useResetPassword() {
  return useApiMutation<unknown, { userId: string }>(
    (vars) => ({ url: `${API_ENDPOINTS.RESET_PASSWORD}/${vars.userId}/reset-password`, method: 'POST' })
  );
}
