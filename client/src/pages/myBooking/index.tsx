  ...
  return (
    <section className="mt-24 px-8 mb-20 mx-auto max-w-7xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-[var(--text-secondary)]"> Manage your venue bookings and track upcoming events. </p>
      </div>

      {/* Search */}
      <div className="relative mb-8">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] size-5" />
        <Input placeholder="Search booking or venue..." className="h-13 rounded-2xl border-[var(--bg-grey)] bg-[var(--bg-tertiary)] pl-12" />
      </div>
      ...
      <div className="space-y-5">
        {filteredBookings.map((booking) => (
          <div key={booking.id} className="flex flex-col md:flex-row overflow-hidden rounded-[28px] border border-[var(--bg-grey)] bg-[var(--bg-tertiary)] shadow-sm">
            {/* image */}
            <div className="relative md:w-[300px] shrink-0">
              <img src={booking.image} alt={booking.venue} className="h-[220px] w-full object-cover md:h-full" />
              {/* status */}
              <div
            ...
            <div className="flex flex-1 flex-col justify-between p-5">
              <div>
                <div className="mb-4 flex items-start justify-between">
                  <div>
                    <h2 className="text-xl font-semibold text-[var(--text-primary)]"> {booking.venue} </h2>
                    <p className="text-sm text-[var(--text-secondary)]"> {booking.place}, {booking.district} </p>
                  </div>
                  <p className="text-xs text-[var(--text-secondary)]"> Booking ID: BMV-{booking.id} </p>
                </div>
                {/* Details */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-[var(--bg-primary)] p-3">
                    <p className="text-xs text-[var(--text-secondary)]"> Date </p>
                    <p className="text-sm font-medium text-[var(--text-primary)]"> {booking.date} </p>
                  </div>
                  ...
                  <div className="rounded-2xl bg-[var(--bg-primary)] p-3">
                    ...
                    <p className="text-sm font-medium text-[var(--text-primary)]"> {booking.guests} </p>
                  </div>

                  <div className="rounded-2xl bg-[var(--bg-primary)] p-3">
                    <p className="text-xs text-[var(--text-secondary)]"> Amount </p>
                    <p className="text-sm font-medium text-[var(--text-primary)]"> ₹{booking.price} </p>
                  </div>
