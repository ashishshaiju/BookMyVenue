      ...
      <div className="grid grid-cols-12 gap-3">
        ...
        <div className="col-span-8">
          <div className=" max-w-4xl ml-4 mb-10 px-4 z-10 font-sans">
            ...
            <div className="mt-8 bg-[var(--bg-tertiary)] border border-[var(--bg-grey)] rounded-3xl p-6">
              <h2 className="text-2xl font-bold text-[var(--text-primary)]">About Venue</h2>
              <p className="mt-4 text-[var(--text-secondary)] leading-8">{venue.description}</p>
            </div>
            {/* Amenities */}
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {venue.amenities?.slice(0, 8).map((item, index) => (
                  <div
                    key={index}
        ...
        <div className="col-span-4">
          <div className="sticky top-28  mr-15 bg-[var(--bg-tertiary)] border border-[var(--bg-grey)] rounded-3xl p-4">
            ...
            {/* slot selection*/}
            <div className="mt-6">
              <h3 className="font-semibold text-[var(--text-primary)] mb-3">Available Slots</h3>

              <div className="grid grid-cols-2 gap-3">
                {/* for fixed slots */}
                {venue.bookingType === 'fixed' &&
                  venue.fixedSlots?.map((item, index) => (
                    <button
                      key={index}
                      onClick={() => {
                ...
                {venue.bookingType === 'flexible' &&
                  venue.generatedSlots?.map((item, index) => (
                    <button
                      key={index}
                      onClick={() => {
                        if (selectedPackage.includes(index)) {
                          setSelectedPackage(selectedPackage.filter((slot) => slot !== index));
                        } else {
                          setSelectedPackage([...selectedPackage, index]);
                        }
                      }}
