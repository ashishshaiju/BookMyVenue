...
import type { BookingCardDTO, BookingDetailDTO, BookerInfoForm, CancelBookingResponse } from '../types/booking.types';

export type ApiResponse<T> = { success: boolean; data: T; message?: string; };

export const getMyBookings = async (): Promise<ApiResponse<{ bookings: { upcoming: BookingCardDTO[], completed: BookingCardDTO[], cancelled: BookingCardDTO[] } }>> => {
...
export const saveBookerDetails = async (payload: BookerInfoForm & { lockId: string }): Promise<ApiResponse<null>> => {
...
export const cancelBooking = async (id: string, reason?: string): Promise<ApiResponse<CancelBookingResponse>> => {
