...
const mockAxiosInstance = Object.assign(
  function (config: Record<string, unknown>) { return mockRequest(config) },
  {
    get: vi.fn(),
    post: vi.fn(),
    put: vi.fn(),
...
describe('useApiQuery', () => {
  ...
  it('should fetch data and unwrap response.data.data', async () => {
    ...
    const { result } = renderHook(
      () => useApiQuery<{ id: number; name: string }>(
        ['test'],
        { method: 'GET', url: '/test' },
      ),
  ...
  it('should accept string query key', async () => {
    ...
    const { result } = renderHook(
      () => useApiQuery('test-key', { method: 'GET', url: '/test' }),
      { wrapper: createWrapper() },
    )
...
describe('useApiMutation', () => {
  ...
  it('should call mutation with config and merge variables as data', async () => {
    ...
    const { result } = renderHook(
      () => useApiMutation({ method: 'POST', url: '/create' }),
      { wrapper: createWrapper() },
    )
  ...
  it('should fail on network error', async () => {
    ...
    const { result } = renderHook(
      () => useApiMutation({ method: 'POST', url: '/fail' }),
      { wrapper: createWrapper() },
    )
