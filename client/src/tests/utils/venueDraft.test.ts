...
describe('venueDraft', () => {
  ...
  describe('saveDraftSession / loadDraftSession', () => {
    ...
    const data = { venueId: 'venue-1', step: 1, formValues: { VenueName: 'Test' } as Record<string, unknown> }
...
describe('clearDraftSession', () => {
  it('should remove draft from sessionStorage', async () => {
    const { saveDraftSession, clearDraftSession, loadDraftSession } = await import('../../utils/venueDraft')
    saveDraftSession(userId, { venueId: 'v1', step: 0, formValues: {} as Record<string, unknown> })
